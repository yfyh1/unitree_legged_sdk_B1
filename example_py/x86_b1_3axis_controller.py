"""
Unitree B1 high-level control wrapper for x86 (3-axis vx, vy, yaw version).
...
class B13AxisController:
    ...
    # 修改：支持vx forward、vy lateral、yaw三轴速度
    def send_motion(self, forward_speed, lateral_speed, yaw_speed):
        ...
        if abs(forward_speed) > 1e-6 or abs(lateral_speed) > 1e-6 or abs(yaw_speed) > 1e-6:
            self.cmd.mode = 2
            # 不再写死横向速度0，传入vy
            self.cmd.velocity = [forward_speed, lateral_speed]
            self.cmd.yawSpeed = yaw_speed

        self.udp.SetSend(self.cmd)
        self.udp.Send()
        self._last_forward = forward_speed
        self._last_lateral = lateral_speed
